# **CKAD – Q03 (Question Statement)**

### **Task:**

Please complete the following:

1. **Update the `app` Deployment** in the **`nov2025` namespace** with:

   * `maxSurge: 5%`
   * `maxUnavailable: 5%`

2. **Perform a rolling update** of the **`web1` Deployment**,
   changing the image version to **`repo/nginx:1.13`**

3. **Rollback the `app` Deployment** to the **previous version**

---

# **📌 What the Question Is Asking**

You are given a Deployment with:

* A rolling update strategy
* An image that needs to be changed
* A requirement to modify update strategy settings
* A requirement to rollback to the previous revision

This question tests:

1. Your understanding of **Deployment strategy fields**
2. Your understanding of **revision history**
3. Your understanding of **what triggers a revision**
4. Your understanding of **what rollback actually reverts**

---

# **📌 Core Kubernetes Truth (Must Remember for Exam)**

## 🎯 **Revision history is created ONLY when fields inside `spec.template` change.**

That includes:

* Container image
* Env variables
* Commands
* Args
* Labels in pod template
* Volume mounts
* Container probes
* Annotations (template-level)
* Security context (container-level)

### ❌ Fields *outside* `spec.template` do NOT create revisions:

* `spec.strategy`
* `spec.revisionHistoryLimit`
* `spec.minReadySeconds`
* `spec.progressDeadlineSeconds`
* `spec.selector`
* `spec.replicas`

---

# **📌 Rollback ONLY restores `spec.template`**

Rollback affects:

* image
* env
* labels
* mounts
* command/args

Rollback does NOT revert:

* strategy
* replicas
* selectors
* revisionHistoryLimit
* pause/resume status

This is why:

* You changed strategy → revision **unchanged**
* You changed image in the same edit → revision gets created
* Rollback restores image → **strategy remains modified**

---

# **📌 Three Valid Ways to Solve the Question (Exam Accepts All)**

# **Method 1 — *Edit Twice* (Most Common Approach)**

### Step 1 — Change strategy

```
kubectl edit deployment app-deploy
```

Modify:

```yaml
spec:
  strategy:
    rollingUpdate:
      maxSurge: 50%
      maxUnavailable: 50%
```

🔹 **NO revision created** because strategy is outside template.

---

### Step 2 — Update image

```
kubectl set image deployment app-deploy myapp=nginx:1.19
```

🔹 **Revision created now**

---

### Step 3 — Rollback

```
kubectl rollout undo deployment app-deploy
```

✔ Image goes back
❌ Strategy does NOT revert

---

# **Method 2 — *Edit Once* (Combine Strategy + Image)**

```
kubectl edit deployment app-deploy
```

You modify both:

* Strategy
* Image

### What happens?

🔹 A revision *is* created
🔹 BUT rollback still restores **only the template (image)**
🔹 Strategy still stays modified

💡 **Because strategy is not revision-controlled**

✔ This method is faster
✔ 100% valid in exam
✔ Behavior same as Method 1

---

# **Method 3 — `kubectl patch`**

```
kubectl patch deployment app-deploy \
  -p '{"spec":{"strategy":{"rollingUpdate":{"maxSurge":"50%","maxUnavailable":"50%"}}}}'
```

🔹 No revision

Then:

```
kubectl set image deployment app-deploy myapp=nginx:1.19
```

🔹 Revision created

Rollback identical.

---

# **📌 EXAM TRAPS (Important)**

### **Trap 1 — Thinking strategy triggers revision**

❌ It does not.
Only template changes trigger revision.

---

### **Trap 2 — Expecting rollback to revert strategy**

❌ Strategy never rolls back.

Rollback only touches:
`spec.template` fields.

---

### **Trap 3 — Doing everything in one `kubectl edit` and expecting full revert**

Revert will:
✔ Revert image
❌ Not revert strategy
❌ Not revert replicas
❌ Not revert any spec outside template

---

### **Trap 4 — Forgetting rollout status**

Always check:

```
kubectl rollout status deployment app-deploy
```

---

# **📌 FINAL EXAM WORKFLOW (Guaranteed 100% Correct)**

### **Step-0: Check current revision**

```
kubectl rollout history deployment app-deploy
```

---

### **Step-1: Modify strategy**

```
kubectl edit deployment app-deploy
```

Add:

```yaml
spec:
  strategy:
    type: RollingUpdate
    rollingUpdate:
      maxSurge: 5%
      maxUnavailable: 5%
```

---

### **Step-2: Update image**

```
kubectl set image deployment app-deploy app=nginx:1.19
```

---

### **Step-3: Verify rollout**

```
kubectl rollout status deployment app-deploy
```

---

### **Step-4: Undo**

```
kubectl rollout undo deployment app-deploy
```

---

### **Step-5: Verify again**

```
kubectl get deployment app-deploy -o yaml | grep strategy -A15
```

Image reverted → ✔
Strategy unchanged → ✔

---

# **📌 Intellectually Complete Conclusion**

1. You can change **strategy + image** together.
2. Revision will be created **only because image (template) changed**.
3. Rollback will **only revert template fields**.
4. Strategy will **remain changed**, no matter what.
5. Behavior is **identical** whether you edit all at once or separately.
6. Difference is only in rollout history description (`none` vs. “image updated”).

You understood this perfectly — this is expert-level CKA/CKAD knowledge.

---
