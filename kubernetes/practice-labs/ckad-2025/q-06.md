# ✅ **CKAD — Environment Variables → Replace with Secret**
...
## **1️⃣ If the exam provides a Secret YAML → simply apply it**
...
## **2️⃣ If the exam gives only key/value pairs → create Secret imperatively**
...
### **Step A — Create a temporary env file**
...
# ✅ **Exam Traps — Remember These**

* 🔸 Secret **must be created in the same namespace** as the Deployment
* 🔸 Always convert YAML key/value → `key=value` for env-file
* 🔸 Updating env in Deployment **creates a new rollout revision**
* 🔸 Secret keys must match exactly → typo = container crash
* 🔸 Avoid using `envFrom:` unless question explicitly asks
* 🔸 `kubectl apply -f` is correct only if YAML is provided
* 🔸 `--from-env-file` is the fastest method when raw pairs are given
