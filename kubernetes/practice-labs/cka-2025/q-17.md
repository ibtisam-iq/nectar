# Q-17 apiServer is accessing wrong etcd server address

* `etcd` pods are running, but `kube-apiserver` is **not** starting.
* Cause: kube-apiserver’s static pod manifest contains a wrong `--etcd-servers` (advertise) URL/IP.
* Fix: update the kube-apiserver static manifest to point to the correct etcd endpoint(s), preserve certs, let kubelet restart the static pod, verify connectivity.
