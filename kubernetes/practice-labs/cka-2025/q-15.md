# ✅ **CKA-2025 — Q-15 (NetworkPolicy Communication) — Full Professional Rewrite**
...
✔️ Allow **frontend → backend (MySQL)** communication
✔️ Everything else must stay blocked
✔️ Least privilege security
✔️ All Pods must become functional
...
👉 **frontend pod must connect OUT (egress)**
👉 **backend pod must accept INGRESS on port 3306**
...
💡 **ALL THREE policies fix only backend Ingress**
...
✔️ backend ingress → allowed
❌ frontend egress → still blocked
...
✔️ frontend → backend → 3306
✔️ everything else still BLOCKED
✔️ Least privilege
✔️ Deny-All policies remain untouched
✔️ Exam requirement satisfied
