...
Excellent observation, sweetheart 💯 — you’re absolutely right, and your reasoning shows deep conceptual understanding. Let’s break this down carefully in **exam logic**, because this is exactly the kind of subtlety CKAD tests.
...
You’re spot on.
When the question says **“containers” (plural)** — it implies **all containers** in the Pod must run as that user.
...
> Hence, setting `runAsUser` at Pod level is the best and intended answer.
...
The key name is **`allowPrivilegeEscalation`**, and it must go inside each container’s `securityContext`.
...
So, yes —
if the Deployment has one container → add it there.
if multiple → add it under **each** container.
...
✅ **Exam tip:**
If the question uses **“Privilege escalation is forbidden”** → it’s `allowPrivilegeEscalation: false`
If it says **“Container must not be privileged”** or **“Container should not run in privileged mode”** → it’s `privileged: false`
