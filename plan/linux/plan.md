...
- **Networking captures:** module 13, its scenario and its lab use a FlexBox playground with `client` (Ubuntu), `gw` (Rocky, router) and `web` (Rocky) on two networks (D43). Output passes through a scrubber for MAC addresses, provider and public NTP addresses and WireGuard keys (D44); lab services are listed in D45.
- **SSH and Security captures:** modules 14 and 15, their six scenarios and the Security lab reuse the batch F FlexBox (D49) with users `ops`, `deploy` and `partner` (D50). The Rocky hosts run enforcing SELinux, firewalld and auditd after installing the security packages and relabeling (D51, D52); AppArmor is described only, because the kernel's active LSM is SELinux (D53), and OpenSCAP CIS rules report `notapplicable` on these microVM images (D54). The scrubber also masks SSH and TLS public keys and kernel-log MAC fields (D55).
- **Boot and Performance captures:** module 17 (performance) is captured on the Rocky `web` host of the reused FlexBox, with `sysstat`, `strace`, `perf`, `stress-ng`, GNU `time` and `tuned` installed; distro tabs cover only install and package differences, because the tools and `/proc` produce identical output across distributions and the Ubuntu `client` has no outbound mirror access (D59, D60). Module 16 (boot and recovery) captures the direct-kernel-boot microVM's `systemd-analyze`, `/proc/cmdline`, a real failed unit, the SysRq and panic sysctls and a real bad-`fstab` error; GRUB, initramfs and `rd.break` are shown as commands without output, since the microVM has no GRUB or initramfs and the kernel is not a package (D61). The OOM kill and exit 137 are captured in a hand-made cgroup, because `systemd-run` scopes are unkillable (D63).

- **Network storage and container captures:** module 18 uses the reused FlexBox with `web` (Rocky) as the NFS/iSCSI/Samba server and `gw` (Rocky) as the client, and the high-load-low-cpu scenario reproduces a real `D`-state NFS hang by partitioning the server with an `nft` rule under a `hard` mount (D67 to D69). Module 19 captures namespaces, cgroup v2 CPU throttling, memory OOM (exit 137) and pids limits, plus rootless Podman uid mapping and overlay copy-up, all on `web` (D70, D72). Module 20 is command-only for VM operations because the microVM has no `/dev/kvm` (`virt-host-validate` FAIL captured), while `qemu-img`, `cloud-init schema`, machine-id and SSH host keys are captured for real (D71).
