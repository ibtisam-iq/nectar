...
**Phase 0: log the plan.** `plan/` (this folder), LLM pointers in `CLAUDE.md`, `GEMINI.md` and a new `AGENTS.md`, `plan/` added to `exclude_docs`, and `scripts/gen-checklist.py`. Delivered as its own pull request, merged to `main` first.
...
5. Results written to `audit-report.md`; `plan/README.md` marks Linux complete; pull request opened with CI preview.
6. Owner sign-off.
