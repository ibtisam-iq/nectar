...
| # | Decision | Reason |
|---|---|---|
| D66 | Batch H approved by the owner ("please continue") | Batch exit criterion |
| D67 | Batch I reuses the batch F/G/H FlexBox (D43/D49/D59), restarted from its preserved state. Module 18 uses `web` (Rocky) as the NFS/iSCSI/Samba server and `gw` (Rocky) as the client, both of which took `nfs-utils`, `autofs`, `targetcli`, `iscsi-initiator-utils`, `samba` and `cifs-utils` from their mirrors; module 19 (containers) is captured on `web`, which took `podman` and whose kernel exposes cgroup v2 with all controllers and the eight namespaces | The topology already has two Rocky hosts on one subnet for a server/client pair, and container primitives need only a single host |
| D68 | Module 18 uses `gw` (Rocky) as the NFS/SMB client rather than the Ubuntu `client`, because the Ubuntu host has no outbound mirror access in this topology (D60) and could not install `nfs-common`/`cifs-utils`. Distro tabs cover only the package-name differences (`nfs-kernel-server`/`nfs-common` vs `nfs-utils`), which is the genuine difference; the client commands and mount output are identical across distributions | The mount and export tooling behaves the same; only package names differ, and those are shown in tabs |
| D69 | The high-load-low-cpu capture was produced by dropping NFS traffic to the server with an `nft` rule while a `hard`-mounted `dd` ran on the client, giving a real `D`-state process with `WCHAN` `rpc_wait_bit_killable` and an `nfsv4` kernel stack, load rising while `vmstat` showed the CPUs 100% idle. The block was removed and the mount drained afterward (NFS `timeo=600` backoff meant recovery took up to a minute) | This is the exact production signature the scenario teaches, and it needed a genuine server partition to reproduce |
| D70 | The cgroups OOM and the high-memory-oom scenario reuse the hand-made-cgroup technique (D63): `memory.max` with `memory.swap.max=0` and `oom_score_adj` reset to 0 produced the real `Memory cgroup out of memory: Killed process ... (python3)` line, exit 137, and `memory.events` `oom_kill 1`. The CPU-throttling capture (`cpu.max` `20000 100000`) gave `nr_throttled 42` of `44` periods, and `pids.max 5` gave a real `fork: retry: Resource temporarily unavailable` with `pids.events max 3` | Every container-limit claim is backed by a real kernel result rather than a described one |
| D71 | Module 20 (virtualization) is command-only for VM operations: the microVM has no `/dev/kvm` and `virt-host-validate qemu` returns `FAIL (Host not compatible with KVM ...)`, captured honestly on the page. Real output is still captured where the tooling does not need KVM: `qemu-img create`/`info`/`convert` (thin qcow2 vs raw), `cloud-init schema` validation (valid and invalid configs), `/etc/machine-id` and SSH host-key fingerprints for the clone-identifier section. `virsh`/`virt-install`/`virt-sysprep`/`virt-clone` and Kickstart are shown as commands without output, with footers stating so | D20 flagged nested KVM as needing a full VM; the plan allows command-only blocks, and the parts that do not need KVM are captured for real |
| D72 | Rootless Podman claims are backed by real captures: `/etc/subuid` `laborant:524288:65536`, a container's `/proc/PID/uid_map` (`0 1001 1` then `1 524288 65536`), the container process found on the host by PID with its own namespace inodes, `--init` showing `podman-init` as PID 1, and an overlay copy-up leaving the lower layer unchanged. SELinux is `Disabled` on the capture host, so `:Z` behaviour is described from the RHEL enforcing default rather than captured | The isolation and mapping mechanics are the interview-weighted content and are all real; only the SELinux label behaviour is unavailable on this host |
| D73 | Batch I pages avoid "Related" links to files that do not exist yet: the Phase 5 `containers-by-hand-lab` is named in prose in the module 19 README without a link, and the module 20 README states its labs come later | The plan forbids forward links to unbuilt files; the audit link check enforces it |

## 2026-09-18: Phase 4 (Interview Layer)

| # | Decision | Reason |
|---|---|---|
| D74 | Batch I approved by the owner ("continue"), then Phase 4 started on the owner's "continue" | Phase entry criterion |
| D75 | Phase 4 adds the two remaining interview pages, `round-2-hands-on.md` (L2 timed tasks plus output-reading drills) and `mock-interviews.md` (three 45-minute mocks mixing L1 to L4). All scenarios were already built through batch I, so no scenario work remained for this phase | The plan's Phase 4 is rounds 2 and 4 content plus mocks; rounds 1/3/4 already exist and grow per batch |
| D76 | The round-2 output-reading drills use fresh real captures from the reused FlexBox (`top` under `stress-ng --cpu`, `free` under a Python allocator at `available` 10Mi, `vmstat` at run-queue 3 to 4, `df` of a 95%-full tmpfs, `iostat` under `stress-ng --hdd`, `ss -tulpn`, and a real `Z` zombie). The `iostat` drill is framed as "disk not the bottleneck" (`%util` 7.57) because this microVM's backing disk stays fast even under heavy write, which is itself a useful read | Every drill snapshot is a genuine capture; the fast disk is stated honestly rather than faked into saturation |
| D77 | `mock-interviews.md` links each question to where it is answered rather than repeating the topic Q&A, per the interview-question rule that `interview/` pages span topics and never copy them. Numbered list items were kept to two prose sentences because the linter counts the `N.` marker as a sentence | Avoids duplicating answers and satisfies the paragraph-length rule for numbered lists |

## 2026-09-18: Phase 5 (Reference and Labs)

| # | Decision | Reason |
|---|---|---|
| D78 | Phase 5 started on the owner's "please continue" after Phase 4 | Phase entry criterion |
| D79 | The six reference pages are built as pure lookup: tables and grouped command blocks, no `??? question` checkpoints, no `Track` line and no capture footer, because they aggregate content already captured on the topic pages and carry no output blocks of their own. `cheatsheet.md` groups commands by task with no collapsibles so it prints in one sweep; `command-index.md` is one A-to-Z table linking each command to its topic; `important-files.md`, `rhel-vs-ubuntu.md` and `glossary.md` are reference tables; `no-tools-fallbacks.md` shows `/proc` and built-in stand-ins as command blocks | Reference pages are the Revise layer (Diataxis reference), so they hold no new captures and no interview Q&A; the audit applies only the generic file rules to them (links, tabs, output honesty) |
| D80 | The two Phase 5 labs match the established lab pattern (task then `??? tip "Solution"` with command blocks, no fabricated output). `rhcsa-style-tasks.md` resolves the INVENTORY rows targeting it (assignments 1 to 3, exam-practice, LVM PE maths) as ten exam-shaped tasks; the root-password-reset procedure is a `Layout:` text block because it is GRUB and initramfs interaction, not shell. `break-fix-lab.md` is SadServers-style: a `Break` command block, a plain-prose symptom, then a solution, with a `danger` admonition that the breaks are destructive | Labs are tutorials, the one place imperative steps are allowed; no output is fabricated, and symptoms are described rather than shown so nothing is invented |
| D81 | The DHCP DORA handshake, deferred from the RHCSA DHCP-server material (INVENTORY row 37), is resolved as one glossary line pointing at the interfaces topic, as that INVENTORY row specified. All other INVENTORY rows were already resolved before Phase 5; the two rows targeting Phase 5 files (`rhcsa-style-tasks.md`, `glossary.md`) now point at existing files | Closes the last two open INVENTORY targets and satisfies the plan's Phase 5 requirement to resolve every INVENTORY item |
| D82 | The reference pages are surfaced in three places: `reference/.pages` (ordered lookup-first), a new "Reference" table in the tool-folder `README.md`, and cross-links in each page's "Related" section. The two labs are added to `labs/.pages`, the `labs/README.md` index and the `roadmap.md` tracker (both ticked) | Discoverability for the Revise and Practice layers; the audit navigation check requires every file to appear in its `.pages` |

## 2026-09-18: Phase 6 (Completion Audit)

| # | Decision | Reason |
|---|---|---|
| D83 | Phase 6 started on the owner's "continue" after Phase 5. The completion audit ran the full `audit-tool.py`, `lychee`, a served rendering pass and an independent fresh-agent review; results are in `plan/linux/audit-report.md` | Phase entry criterion and the plan's Phase 6 definition |
| D84 | The `--full` audit's only failures are check 15 (unticked checklist boxes), which are the pending-owner boxes and the Phase 6 steps themselves. This is circular by construction, so `p6:audit-full` and `p6:owner-signoff` stay unticked until the owner closes the final boxes; checks 1 to 14 and 16 pass with 0 failures | The final audit can reach 0 only after owner sign-off, so it is honestly recorded as residual rather than forced to green |
| D85 | `lychee` reports 8 broken targets (`compose.yml`, `Chart.yaml`, `k8s.txt`, `efs-fsx.md`, `secret-manager.md`, `rs.guide.yaml`, `volumes.yaml`, a CKA PDF) from the global Material nav sidebar, rendered site-wide for top-level repo files excluded from the build. Left unfixed and recorded for the owner, because the fix is in the global navigation, outside this plan's scope, and CLAUDE.md says not to restructure navigation unless asked. Zero broken links exist inside Linux content | Out of scope for the Linux rebuild; documented rather than silently changed |
| D86 | Independent-review findings were resolved: the `rhel-vs-ubuntu` `useradd` accuracy error and the borderline `tar -f` recall question were fixed, three minor recall L1s (`pam`, `awk`, `login-sessions`) were reworded to reasoning prompts, the `no-tools-fallbacks` glob label and `break-fix` tmpfs exercise were corrected, and the `round-1` separators and `round-4` duplicate phrasing were tidied. Two L1s (`exit-codes` 128+n reasoning, `text-editors` vim save) were kept with reason, and the `flatpak-and-snap` curly quote was kept as a literal error string | Findings fixed or accepted with a stated reason, per the plan's "findings are fixed and re-run until clean" |
| D87 | `plan/README.md` marks Linux "Complete: audit clean, pending owner sign-off" rather than a bare "complete", since the definition of done ties completion to a clean `audit-report.md` (now written) while the final owner sign-off is a separate last gate | Honest status that satisfies the Phase 6 "mark complete" step without overclaiming owner approval |

## 2026-09-19: Post-merge fix (closeout PR #20)

| # | Decision | Reason |
|---|---|---|
| D88 | The Codex reviewer on PR #20 found that `check_coverage()` requires `_sources/INVENTORY.md`, but `_sources/` is git-ignored, so `audit-tool.py --full` fails check 9 on a fresh checkout and the ticked "`--full` exits 0" claim was true only on the authoring machine. Fixed by force-tracking `INVENTORY.md` (`git add -f`) while `_sources/` stays git-ignored for the raw material; `exclude_docs` still keeps it out of the site. Updated `.gitignore` comment, `INVENTORY.md` header, `plan.md`. The `_sources/` gitignore token is unchanged, so audit check 13 still passes | Makes the full-audit-clean claim reproducible from the repository, which is the point of a completion audit; keeps raw material unpublished |
