...
## 2026-09-24: Phase 5 (reference and labs)

| # | Decision | Reason |
|---|---|---|
| G27 | Phase 5 built the five reference pages (`cheatsheet` grouped by task with no collapsibles, `command-index` A-Z with plumbing marked, `config-reference` keyed by area with level and default, `dotfiles-reference` absorbing the `gitattributes-sample`, `glossary` alphabetical) and the two remaining labs (`internals-by-hand-lab` building a two-commit history with only plumbing, `history-surgery-lab` squashing, amending, purging a secret and force-with-lease). Reference pages that show only `bash` blocks carry no capture footer (per the audit's Output rule); the two labs show real captured output and carry footers | The plan schedules these for phase 5; the reference pages are lookup material (commands and formats), so most need no captured output, while the labs are worked exercises whose solutions show real git 2.50.1 output |
| G28 | Every `_sources/INVENTORY.md` target now exists (verified: all 20 referenced files present, including the four reference pages the cheatsheet and dotfiles items mapped to). No INVENTORY row needed a status change; the `dropped`, `fact` and `deferred` rows stand as recorded | The `p5:inventory-resolved` checklist item requires each mapped item to have a live target; the mapping was correct from Phase 1, and Phase 5 built the last targets it pointed at |
| G29 | The `history-surgery-lab` records that `--force-with-lease` is refused with `(stale info)` immediately after `filter-repo` re-adds no remote-tracking ref, and succeeds only after a `git fetch` establishes one. This real two-step capture is kept rather than smoothed over | It is honest captured behaviour and a genuine teaching point: the lease compares against a remote-tracking ref, so a freshly re-added remote must be fetched before the lease has anything to check |

---

## 2026-09-24: Phase 6 (completion audit)

| # | Decision | Reason |
|---|---|---|
| G30 | Phase 6 ran the completion audit: `audit-tool --full` (only check-15 circular boxes fail), `lychee` (0 broken links inside git; the 8 global-nav targets are pre-existing and out of scope, as in the Linux audit), a served desktop and phone rendering pass, and an independent fresh-agent review. The review found no must-fix errors; its one should-fix (bisect exit-code range) and five polish items were all fixed, and three captured-output blocks (trunk-based squash log, detached-HEAD reflog, rerere preimage/resolution) were re-captured from real git 2.50.1 for honesty. `plan/git/audit-report.md` records the result. The tool is left "awaiting owner sign-off and merge" rather than "complete", since completion needs the owner's push and sign-off | The plan's Phase 6 requires the full audit, link and render checks, an independent review with findings fixed, and the report; the "complete" status in `plan/README.md` is reserved for after the owner signs off and merges, matching how Linux was marked |
