# Foundations

This section collects the mental models and fundamental knowledge I use repeatedly as an engineer.
Use these pages to quickly refresh a concept or to get the minimal, practical view — not long tutorials.

## Quick links
- [SDLC](SDLC.md)
- [Programming Basics](programming.md)
- [Frameworks Overview](frameworks.md)
- [Databases (Conceptual)](database.md)
- [CLI Fundamentals](understanding-CLI-commands.md)

### Linux
- [Linux Overview](../linux/Linux.md)
- [STDOUT & STDERR](../linux/STDOUT_STDERR_Guide.md)
- [Cheat Sheet](../linux/cheatSheet.md)
- [tar Command](../linux/tar-command.md)
- [Troubleshooting](../linux/troubleshooting.md)

### Networking
- [Networking Basics](../networking/Networking.md)
- [DNS Resolution (curl)](../networking/curl-dns-resolution.md)

### Other tools
- [Bash Notes](../bash/ibtisam.sh.md)
- [Vimrc Guide](../vim/vimrc_guide.md)
